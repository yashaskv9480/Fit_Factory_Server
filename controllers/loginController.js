...
exports.user_login = async (req, res) => {
  try {
    ...
    const userDetails = await db.query(
      "SELECT user_id,name FROM user_details WHERE email=($1) AND password=($2)",
      [email, password],
    );
    ...
    if (user) {
      ...
      if (userRole == "client") {
        const gymDetailsResult = await db.query(
          "Select gym_id,gym_name FROM gym_details WHERE user_id = $1",
          [user.user_id],
        );
        const gym = gymDetailsResult.rows[0];
        token = await jwtTokenGeneration.token_generation(
          user.user_id,
          user.name,
          userRole,
          gym.gym_id,
