...
exports.viewUsers = async (req, res) => {
  try {
    const getUsersResult =
      await db.query(`SELECT ud.user_id, ud.name, ud.email, ud.mobile
...
exports.viewClients = async (req, res) => {
  try {
    const getClientResult =
      await db.query(`SELECT ud.user_id,gd.gym_id,ud.name, ud.email, ud.mobile, gd.gym_name, gd.location, gd.gym_price
        FROM user_details ud
        JOIN user_role_management urm ON ud.user_id = urm.user_id
...
exports.viewAllBookings = async (req, res) => {
  try {
    const getAllBookingsResult =
      await db.query(`SELECT ud.name, gd.gym_name, b.booking_id, b.booking_date, b.amount
        FROM bookings b
