...
async function userSignup(name, email, password, mobile, roleId) {
  const userQuery =
    "INSERT INTO user_details(name, email, password, mobile) VALUES ($1, $2, $3, $4) RETURNING user_id";
  const userValues = [name, email, password, mobile];
  const userResult = await db.query(userQuery, userValues);
  const user_id = userResult.rows[0].user_id;

  const roleQuery =
    "INSERT INTO user_role_management(user_id, role_id) VALUES ($1, $2)";
...
exports.client_signup = async (req, res) => {
  try {
    ...
    if (check_user_id) {
      ...
      const { gym_name, description, location, address, timings, gym_price } =
        req.body;
      const gymQuery =
        "Insert into gym_details(user_id,gym_name,description,location,address,timings,gym_price) values ($1,$2,$3,$4,$5,$6,$7) returning gym_id";
      const gymValues = [
        user_id,
        gym_name,
        description,
      ...
      const gymImageResult = await db.query(
        "Update gym_details SET gym_image = $1 where gym_id = $2",
        [imageName, gym_id],
      );
...
exports.google_oauth = async (req, res) => {
  ...
  try {
    ...
    if (user_id) {
      ...
      const token = await jwtTokenGeneration.token_generation(
        user_id,
        name,
        userRole,
      );
