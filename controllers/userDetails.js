...
exports.checkuserRole = async (user_id) => {
  const userRoleId = await db.query(
    "SELECT role_id FROM user_role_management WHERE user_id = $1",
    [user_id]
  );
  const userRole = await db.query("Select role FROM roles where role_id = $1", [
...
exports.checkuserId = async (email) => {
  const userQuery = await db.query(
    "SELECT user_id FROM user_details WHERE email = $1",
    [email]
  );
